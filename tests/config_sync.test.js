...
test('resendDict: saved config -> dict with parsed SortOrder', () => {
  assert.deepStrictEqual(
    resendDict(store({ timer_config: 'Egg\x1f300\x1eTea\x1f120', sort_order: '1' })),
    { TimerConfig: 'Egg\x1f300\x1eTea\x1f120', SortOrder: 1 });
});

test('resendDict: explicitly-saved empty list ("") IS sent (user cleared all timers)', () => {
  assert.deepStrictEqual(
    resendDict(store({ timer_config: '', sort_order: '0' })),
    { TimerConfig: '', SortOrder: 0 });
});
